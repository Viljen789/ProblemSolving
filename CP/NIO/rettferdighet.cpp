...
// Add your solution herecc
	...
	for (int i = 1; i < n; ++i){
		// totsums[i] = totsums[i-1]+(partsums[0][i])*(i>=m)+abs((partsums[0][i]))*(i<m);
	...
	for (int i = 0; i < m; ++i) {
	    ...
	    for (int j = 0; j < m; ++j) {
	        ...
	        while (!temp.empty()) {
	        	if(temp.top()<0&&i!=m-1)dp[i][j].s+=temp.top();
				else dp[i][j].f += temp.top();
				dp[i][j].s = totsums[j]-dp[i][j].f;
	            temp.pop();
	        }
	    }
	}
	...
	// for (int i = 0; i < m; ++i)
	// {
	// 	dp[i][m-1].f = dp[i][m-2].f - totneg;
	// }
	// GÅ TIL DEN NESTE POSITIVE VERDIEN OG LEGG TIL den negative verdien der. 
	...
	for (int i = 0; i < m-1; ++i)
	{
		...
		{
			if(partsums[0][j]<0){
				dp[i][j].f+=partsums[0][j];
				dp[i][j].s += partsums[0][j];
			}
		}
	}
	// for (int i = 0; i < m; ++i)
	// {
		// if(dp[i][m-1].s>dp[i][m-1].f)swap(dp[i][m-1].s, dp[i][m-1].f);
	// }
	// db(dp);
	...
	// cout<< totneg;
	cout << dp<<endl;
	cout << dp[m-1][n-1].f<<" "<<dp[m-1][n-1].s+totneg;
}
