...
// Add your solution herecc
	...
	totsums[0] = abs(partsums[0][0]);
	if(partsums[0][0]<0)totneg = totsums[0];
	for (int i = 1; i < n; ++i){
		totsums[i] = totsums[i-1]+abs(partsums[0][i])*(i<m)+(partsums[0][i])*(i>=m);
		totneg += partsums[0][i]*(i<m&&partsums[0][i]<0);
		// negvec[i] = negvec[i-1];
		// negvec[i] += partsums[0][i]*(i<m&&partsums[0][i]<0);
	}
	// cout << negvec;
	// cout << partsums;
	...
	for(int i = 0; i<n; i++){
        for(int j = 1; j<=m; j++){
            if(!partsums[j][i])partsums[j][i] = max(totsums[i]-totsums[i-j], partsums[j][i-1]);
        }
    }
    // cout << partsums;
	for (int i = 0; i < m; ++i) {
	    ...
	    for (int j = 0; j < m; ++j) {
	        ...
	        while (!temp.empty()) {
				...
				dp[i][j].f += abs(temp.top());
				if(dp[i][j].f + temp.top()==abs(negvec[i]))dp[i][j].f = abs(negvec[i]);
				// dp[i][j].f = max(dp[i][j].f, tot);
				// deb2(dp[i][j-1].f, temp.top()); 

				// if(negvec[i]==-temp.top())dp[i][j].f+=2*abs(temp.top());
				dp[i][j].s = totsums[i]-dp[j][i].f;
				// dp[i][j].s += temp.top()*(temp.top()<0);
	            // dp[i][j].f += temp.top();
	            // deb2(temp.top(), dp[i][j].f);
	            // deb3(temp.top(), i, j);
	...
	// cout << dp<<endl;
	// cout << totneg;
	cout << dp[m-1][n-1].f+totneg<<" "<<dp[m-1][n-1].s+totneg;
