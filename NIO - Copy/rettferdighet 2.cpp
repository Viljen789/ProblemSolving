...
// Add your solution herecc
	...
	for (int i = 0; i < n; ++i)
	{
		if(partsums[0][i]>0||partsums[0][i+1]>0)break;
		else {
			partsums[0][i+1] += partsums[0][i];
			n--;
		}
	}
	for (int i = 0; i < n; ++i)
	{
		partsums[0][i] = partsums[0][i+orgn-n];
	}
	// deb2(partsums, n);
	for (int i = n-1; i >= 0; --i)
	{
		// deb3(i, n-m, partsums[0][i]);
		t = partsums[0][i];
		if(i<m&&t<0)totneg+=t;
		if(t<0){
			if(negind == -1)negind = i;
			if(i!=0&&i<m)onlylast = false;
		}
		else negind = -1;
	}
	...
	// deb3(negind, totneg, onlylast);	
	if(onlylast) {
    partsums[0][0] = abs(partsums[0][0]);
	...
	for (int i = m; i < n; i++)
	{
		if(negind!=-1&&m>1&&negind!=m-1&&!onlylast){
	...
	db(dp);
	// deb3(partsums[0][m-1], partsums[0][0], totneg);
	if(partsums[0][m-1]<0&&totneg==(partsums[0][m-1] + partsums[0][0]))cout <<  dp[m-2][n-1].f<<" "<<dp[m-2][n-1].s;
	else if(!onlylast)cout << dp[m-1][n-1].f<<" "<<dp[m-1][n-1].s;
	else cout << dp[m-1][n-1].f+totneg<<" "<<dp[m-1][n-1].s+totneg;
	// else
